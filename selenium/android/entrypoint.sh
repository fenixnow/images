...
ANDROID_AVD_HOME=/root/.android/avd DISPLAY="$DISPLAY" emulator ${EMULATOR_ARGS} -timezone Europe/Moscow -verbose -writable-system -no-boot-anim -no-audio -no-snapshot-save -avd @AVD_NAME@ -sdcard /sdcard.img -skin "$SKIN" -gpu swiftshader_indirect &
EMULATOR_PID=$!
